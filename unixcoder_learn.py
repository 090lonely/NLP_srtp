...
class UniXcoder(nn.Module):#类定义
    def __init__(self, model_name):
        ...
        self.tokenizer = RobertaTokenizer.from_pretrained(model_name)#创建了一个 RobertaTokenizer 对象，用于对输入文本进行分词和编码。
    ...
    def tokenize(self, inputs, mode="<encoder-only>", max_length=512, padding=False):#将输入的字符串转换为标记（token）的 ID 序列。
        """ 
